...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/cars/{{ $car->id }}" class="mb-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            <label for="merek" class="form-label">Merek</label>
            <input type="text" class="form-control @error('merek') is-invalid @enderror" id="merek" name="merek"
                required autofocus value="{{ old('merek', $car->merek) }}">
            @error('merek')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('model') is-invalid @enderror" id="model" name="model"
                required autofocus value="{{ old('model', $car->model) }}">
            @error('model')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('noplat') is-invalid @enderror" id="noplat" name="noplat"
                required autofocus value="{{ old('noplat', $car->noplat) }}">
            @error('noplat')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('tarif') is-invalid @enderror" id="tarif" name="tarif"
                required autofocus value="{{ old('tarif', $car->tarif) }}">
            @error('tarif')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('stok') is-invalid @enderror" id="stok" name="stok"
                required autofocus value="{{ old('stok', $car->stok) }}">
            @error('stok')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
