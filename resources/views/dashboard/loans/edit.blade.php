...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Edit Loan</h1>
</div>

<div class="col-lg-8">
    <form method="POST" action="/dashboard/loans/{{ $loan->id }}" class="mb-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            ...
            <select class="form-select" name="category_id">
                @foreach ($cars as $car)
                @if (old('car_id', $car->category_id) == $car->id)
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tgl_mulai') is-invalid @enderror" id="tgl_mulai" name="tgl_mulai"
                required autofocus value="{{ old('tgl_mulai', $loan->tgl_mulai) }}">
            @error('tgl_mulai')
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tgl_selesai') is-invalid @enderror" id="tgl_selesai" name="tgl_selesai"
                required autofocus value="{{ old('tgl_selesai', $loan->tgl_selesai) }}">
            @error('tgl_selesai')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
